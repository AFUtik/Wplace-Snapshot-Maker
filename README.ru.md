...
Рекомендуется скопировать репозиторий через команду `git clone https://github.com/AFUtik/Wplace-Shapshot-Maker.git`, либо скачать архив репозитория и разархивировать его. После того как скачали репозиторий, вам требуется запустить start.bat файл на Windows, или start.sh файл , если у вас система Linux. Дальше выполнятся установки требуемых библиотек в корне проекта. После запуска сервера вам нужно ввести `localhost:3000` в ip поле браузера, для того, чтобы перейти на сам сайт. Задать свой порт можно в `settings.json`. 
